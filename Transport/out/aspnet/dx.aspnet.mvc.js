...
!function (factory) {
    ...
    function createTemplateCompiler() {
        var OPEN_TAG = "<%", CLOSE_TAG = "%>", ENCODE_QUALIFIER = "-", INTERPOLATE_QUALIFIER = "=";
        ...
        function acceptCode(bag, code) {
            var encode = code.charAt(0) === ENCODE_QUALIFIER, value = code.substr(1), interpolate = code.charAt(0) === INTERPOLATE_QUALIFIER;
        ...
        return function (text) {
            var bag = ["var _ = [];", "with(obj||{}) {"], chunks = text.split(OPEN_TAG);
    ...
    function createComponent(name, options, id, validatorOptions) {
        var render = function (_, container) {
            var selector = "#" + id.replace(/[^\w-]/g, "\\$&"), $component = $(selector, container)[name](options);
    ...
    return {
        ...
        getEditorValue: function (inputName) {
            ...
            if ($widget.length) {
                var dxComponents = $widget.data("dxComponents"), widget = $widget.data(dxComponents[0]);
                if (widget) {
